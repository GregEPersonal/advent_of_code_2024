...
def vet_line_wrapper(int_array, top_level = true)
  result = vet_line(int_array)

  # If it fails, recursively check the array without each number
  # Actually, don't need recursion. Just run it a number of times
  if !result && top_level
    approved = false
    int_array.each_with_index do |num, index|
      approved |= vet_line(int_array[0...index] + int_array[(index + 1)..-1])
    end
    return approved
  end
  return result
