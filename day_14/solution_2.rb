...
number_seconds = 10403
# show_grid_for_seconds = 20

(6980..number_seconds).each do |second|
  # Set the whoe robot_future_locations grid to 0

  robot_future_locations.each_with_index do |row, i|
    row.each_with_index do |cell, j|
      robot_future_locations[i][j] = 0
    end
  end

  robots.each do |x, y, vx, vy|
    new_x = (x + second * vx) % width
    new_y = (y + second * vy) % height
  ...
  #   puts "ANSWER: #{quadrant_1 * quadrant_2 * quadrant_3 * quadrant_4}"
  # end

  # Show the grid for soem time only
  # if (number_seconds - second < show_grid_for_seconds)
  puts "Second: #{second}"
  ...
  # Then wait .05 seconds
  sleep(0.15)
end

# Okay, new plan. This should loop every 101 * 103 = 10403 iterations.
# We've used two guesses so far - 200, and 1000. It's higher for both.
# WeSHOULD have 3 guesses left. Let's do - 5200. And then either
# IT's higher than 5200
# Next up - 7900 - Didn't give an up or down
# Actual: 7286
